...
{{if $param.IsNestedProto3}}
	...
	{{if $enum}}
		e{{if $param.IsRepeated}}s{{end}}, err = {{$param.ConvertFuncExpr}}(val{{if $param.IsRepeated}}, {{$binding.Registry.GetRepeatedPathParamSeparator | printf "%c" | printf "%q"}}{{end}}, {{if ne $param.Target.Message.File.GoPkg.Path .Method.Service.File.GoPkg.Path}}{{$param.Target.Message.File.GoPkg.Path}}.{{end}}{{$enum.GoType $param.Target.Message.File.GoPkg.Path}}_value)
	{{end}}
{{else if $enum}}
	e{{if $param.IsRepeated}}s{{end}}, err = {{$param.ConvertFuncExpr}}(val{{if $param.IsRepeated}}, {{$binding.Registry.GetRepeatedPathParamSeparator | printf "%c" | printf "%q"}}{{end}}, {{if ne $param.Target.Message.File.GoPkg.Path .Method.Service.File.GoPkg.Path}}{{$param.Target.Message.File.GoPkg.Path}}.{{end}}{{$enum.GoType $param.Target.Message.File.GoPkg.Path}}_value)
{{else}}
...
{{if and $enum $param.IsRepeated}}
	s := make([]{{if ne $param.Target.Message.File.GoPkg.Path .Method.Service.File.GoPkg.Path}}{{$param.Target.Message.File.GoPkg.Path}}.{{end}}{{$enum.GoType $param.Target.Message.File.GoPkg.Path}}, len(es))
	for i, v := range es {
		s[i] = {{if ne $param.Target.Message.File.GoPkg.Path .Method.Service.File.GoPkg.Path}}{{$param.Target.Message.File.GoPkg.Path}}.{{end}}{{$enum.GoType $param.Target.Message.File.GoPkg.Path}}(v)
	}
	{{$param.AssignableExpr "protoReq"}} = s
{{else if $enum}}
	{{$param.AssignableExpr "protoReq"}} = {{if ne $param.Target.Message.File.GoPkg.Path .Method.Service.File.GoPkg.Path}}{{$param.Target.Message.File.GoPkg.Path}}.{{end}}{{$enum.GoType $param.Target.Message.File.GoPkg.Path}}(e)
{{end}}
	{{end}}
{{end}}
{{if .HasQueryParam}}
